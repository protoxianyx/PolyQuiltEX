...
class PQ_Gizmo_Preselect(bpy.types.Gizmo):
    ...
    def get_keyitem(self, shift, ctrl, alt, oskey):
        keymap = bpy.context.window_manager.keyconfigs.user.keymaps[
            "3D View Tool: Edit Mesh, " + self.tool.bl_label
        ]
        keyitems = [
            item for item in keymap.keymap_items if item.idname == "mesh.poly_quilt"
        ]
        for item in keymap.keymap_items:
            if item.idname == "mesh.poly_quilt" and item.active:
                if [item.shift, item.ctrl, item.alt, item.oskey] == [
                    shift,
                    ctrl,
                    alt,
                    oskey,
...
all_gizmos = (
    ...
    PQ_GizmoGroup_Lowpoly,
    PQ_GizmoGroup_Knife,
    PQ_GizmoGroup_Delete,
    PQ_GizmoGroup_Extrude,
    PQ_GizmoGroup_LoopCut,
    PQ_GizmoGroup_Brush,
    PQ_GizmoGroup_Seam,
)
